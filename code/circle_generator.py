        ...
        if (self.center[0] > 0 and self.center[1] > 0 and radius > 0):
            if (noise == False):
                ...
                for i in range(POINTS_NUM):
                    self.points.append(
                        [int(self.center[0] + (self.radius + AMPLITUDE) * np.cos((2 * np.pi / POINTS_NUM) * i)),
                         int(self.center[1] + (self.radius + AMPLITUDE) * np.sin((2 * np.pi / POINTS_NUM) * i))])
                    self.points.append([int(self.center[0] + (self.radius - AMPLITUDE) * np.cos(
